...
extern void cm_pwlts(Mif_Private_t *);
/*.......1.........2.........3.........4.........5.........6.........7.........8
================================================================================
...
static void
cm_pwlts_callback(Mif_Private_t *mif_private, Mif_Callback_Reason_t reason)
{
    switch (reason) {
        case MIF_CB_DESTROY: {
            double *last_x_value =  mif_private->inst_var[0]->element[0].pvalue;
            double *x =  mif_private->inst_var[1]->element[0].pvalue;
            double *y =  mif_private->inst_var[2]->element[0].pvalue;
            if (last_x_value)
                free(last_x_value);
            if (x)
                free(x);
            if (y)
             ...
             mif_private->inst_var[1]->element[0].pvalue = NULL;
             mif_private->inst_var[2]->element[0].pvalue = NULL;
            break;
        }
    }
}
...
void cm_pwlts(Mif_Private_t *mif_private)  /* structure holding parms,
                                   inputs, outputs, etc.     */
{
    ...
    *(mif_private->callback) = cm_pwlts_callback;
    ...
    size = mif_private->param[0]->size + 2;
    ...
    */
    if (mif_private->circuit.init==1) {
        ...
        for (i=1; i<size-1; i++) {
            x[i] =   mif_private->param[0]->element[i-1].rvalue;
            y[i] =   mif_private->param[1]->element[i-1].rvalue;
        }
        /* Add additional leading and trailing values */
        x[0] = 2. * x[1] - x[2];
        x[size - 1] = 2. * x[size - 2] - x[size - 3];
        if (mif_private->param[4]->element[0].bvalue == MIF_TRUE) {
    ...
    x_input = mif_private->circuit.time;
