...
//
///* 提取电路元件和节点信息的函数 */
//CircuitElements get_circuit_elements_nodes(CKTcircuit *ckt) {
...
//
//    /* 初始化元件和节点指针 */
//    elements.instances = NULL;
//    elements.nodes = NULL;
//
//    printf("get_circuit_elements_nodes1\n");
//    /* 遍历所有模型 */
//
//    for (mod = ckt->CKThead[0]; mod != NULL; mod = mod->GENnextModel) {
//        /* 遍历每个模型的元件实例 */
//        printf("for (mod = ckt->CKThead[0]; mod != NULL; mod = mod->GENnextModel) \n");
//        if (mod != NULL && mod->GENinstances != NULL) {
//            printf("if (mod != NULL)\n");
//            for (inst = mod->GENinstances; inst != NULL; inst = inst->GENnextInstance) {
//                /* 添加到元件实例列表 */
//                printf("for (inst = mod->GENinstances; inst != NULL; inst = inst->GENnextInstance) \n");
//                inst->GENnextInstance = elements.instances;
//                elements.instances = inst;
//            }
//        }else if ((mod == NULL)){
//            // 处理mod为NULL的情况
//            printf("(mod == NULL)\n");
...
/*                        eq(s, ".save") || add .save only after subcircuit expansion */
        ...
        //1可能就是这两步
        printf("ckt pointer is %p\n", (void *)ft_curckt);
        if (ft_curckt != NULL) {
            printf("1ft_curckt->ci_ckt->CKTnodes is %p\n", (void *)ft_curckt->ci_ckt->CKTnodes->next);
            printf("1ft_curckt->ci_ckt->CKThead is %p\n", (void *)ft_curckt->ci_ckt->CKThead);
            printf("1ft_curckt->ci_ckt->CKThead[0] is %p\n", (void *)ft_curckt->ci_ckt->CKThead[0]);
            // 检查和打印其他相关状态...
        }
...
//        printf("test\n");
    ...
    printf("ckt pointer is %p\n", (void *)ft_curckt);
    if (ft_curckt != NULL) {
        printf("ft_curckt->ci_ckt->CKTnodes is %p\n", (void *)ft_curckt->ci_ckt->CKTnodes);
        printf("ft_curckt->ci_ckt->CKThead is %p\n", (void *)ft_curckt->ci_ckt->CKThead);
        printf("ft_curckt->ci_ckt->CKThead[0] is %p\n", (void *)ft_curckt->ci_ckt->CKThead[0]);
        // 检查和打印其他相关状态...
    }
    ...
    tfree(dir_name);
//    if(ft_curckt)
//        get_circuit_elements_nodes(ft_curckt);
    printf("inp_spsource end\n");
    return 0;
}//inp_spsource_end
