...
static void callback(Mif_Private_t *mif_private, Mif_Callback_Reason_t reason)
{
    if (reason == MIF_CB_DESTROY) {
        State_Table_t    *table;

        table = (State_Table_t *)mif_private->inst_var[0]->element[0].pvalue;
        if (!table)
            return;
        if (table->state)
            free(table->state);
...
void cm_d_state(Mif_Private_t *mif_private) 
{
	...
	/* Allocate storage for the state transition table. */
        ...
        table->num_inputs = mif_private->conn[0]->size;
        table->num_outputs = mif_private->conn[3]->size;
        ...
        table->next_state = (int *)calloc((size_t)(table->depth + 1),
                                          sizeof (int));
        *(mif_private->callback) = callback;    // To free those allocations.
