...
//
//    /* 遍历所有模型 */
//    for (GENmodel *mod = ckt->CKThead[0]; mod != NULL; mod = mod->GENnextModel) {
//        /* 遍历每个模型的元件实例 */
//        for (GENinstance *inst = mod->GENinstances; inst != NULL; inst = inst->GENnextInstance) {
//            /* 创建一个新的元件实例并添加到元件实例列表 */
//            GENinstance *new_inst = malloc(sizeof(GENinstance));
//            if (new_inst == NULL) {
//                fprintf(stderr, "Error: Memory allocation failed.\n");
//                exit(EXIT_FAILURE);
//            }
//            *new_inst = *inst; // 复制元件实例信息
//            new_inst->GENnextInstance = elements.instances;
//            elements.instances = new_inst;
//        }
//    }
//
//    /* 遍历所有节点 */
//    for (CKTnode *node = ckt->CKTnodes; node != NULL; node = node->next) {
//        /* 创建一个新的节点并添加到节点列表 */
//        CKTnode *new_node = malloc(sizeof(CKTnode));
//        if (new_node == NULL) {
//            fprintf(stderr, "Error: Memory allocation failed.\n");
//            exit(EXIT_FAILURE);
//        }
//        *new_node = *node; // 复制节点信息
//        new_node->next = elements.nodes;
//        elements.nodes = new_node;
//    }
//
//    return elements;
//}
...
/* 提取电路节点信息的函数 */
CircuitElements get_circuit_nodes(CKTcircuit *ckt) {
    printf("get_circuit_nodes\n");
    if (ckt == NULL) {
        fprintf(stderr, "Error: CKTcircuit pointer is NULL.\n");
        exit(EXIT_FAILURE);
    }

    printf("ckt pointer is %p\n", (void *)ckt);
    if (ckt != NULL) {
        printf("ckt->CKTnodes is %p\n", (void *)ckt->CKTnodes);
        // 检查和打印其他相关状态...
    }

    CircuitElements elements;
    elements.instances = NULL; // 如果不需要实例信息，可以保留此行但不使用，或者从结构中移除
    elements.nodes = NULL;

    /* 遍历所有节点 */
    for (CKTnode *node = ckt->CKTnodes; node != NULL; node = node->next) {
        printf("for (CKTnode *node = ckt->CKTnodes; node != NULL; node = node->next)\n");
        /* 创建一个新的节点并添加到节点列表 */
        CKTnode *new_node = malloc(sizeof(CKTnode));
        if (new_node == NULL) {
            fprintf(stderr, "Error: Memory allocation failed.\n");
