...

{
    ...
    if (val == *out) {
        ...
        /* fall to zero value */
        case 0: mif_private->conn[1]->port[0]->delay = mif_private->param[1]->element[0].rvalue;
                break;

        /* rise to one value */
        case 1: mif_private->conn[1]->port[0]->delay = mif_private->param[0]->element[0].rvalue;
                break;
