...
static void cm_table3D_callback(Mif_Private_t *mif_private,
        Mif_Callback_Reason_t reason)
{
    switch (reason) {
        case MIF_CB_DESTROY: {
            Table3_Data_t *loc = mif_private->inst_var[0]->element[0].pvalue;
            if (loc) {
                free_local_data(loc);
                mif_private->inst_var[0]->element[0].pvalue = loc = NULL;
            }
            break;
...
void cm_table3D(Mif_Private_t *mif_private) /* structure holding parms, inputs, outputs, etc. */
{
    ...
    if (mif_private->circuit.init == 1) { /* Must do initializations */
        mif_private->inst_var[0]->element[0].pvalue = init_local_data
                (mif_private->param[4]->element[0].svalue,
                mif_private->param[0]->element[0].ivalue);
        *(mif_private->callback) = cm_table3D_callback;
    }

    /* return immediately if there was an initialization error */
    if ((loc = mif_private->inst_var[0]->element[0].pvalue) == (Table3_Data_t *) NULL) {
        return;
    }
    ...
    if (mif_private->circuit.anal_type != MIF_AC) {
        ...
        xderiv = xramp * mif_private->param[3]->element[0].rvalue * derivval[0] / xdiff;
         mif_private->conn[3]->port[0]->partial[0].port[0] = xderiv;
        yderiv = yramp * mif_private->param[3]->element[0].rvalue * derivval[1] / ydiff;
         mif_private->conn[3]->port[0]->partial[1].port[0] = yderiv;
        zderiv = zramp * mif_private->param[3]->element[0].rvalue * derivval[2] / zdiff;
        ...
        ac_gain.real = xramp * mif_private->param[3]->element[0].rvalue * derivval[0] / xdiff;
        ac_gain.imag= 0.0;
         mif_private->conn[3]->port[0]->ac_gain[0].port[0] = ac_gain;
        ac_gain.real = yramp * mif_private->param[3]->element[0].rvalue * derivval[1] / ydiff;
        ac_gain.imag= 0.0;
         mif_private->conn[3]->port[0]->ac_gain[1].port[0] = ac_gain;
        ac_gain.real = zramp * mif_private->param[3]->element[0].rvalue * derivval[2] / zdiff;
        ac_gain.imag= 0.0;
         mif_private->conn[3]->port[0]->ac_gain[1].port[0] = ac_gain;
    }
